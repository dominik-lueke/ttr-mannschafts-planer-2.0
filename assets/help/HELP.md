...
Die Aufstellungen vergangener Halbserien, TTR-Werte und Bilanzen können manuell von den `click-TT` Seiten übernommen werden. 
Für die eingebetteten Inhalte wird keine Haftung übernommen. 
Für einige Funktionen ist ein Account notwendig.
...
Der Verein sollte genau so eingegeben werden, wie er auf der `click-TT` Info Seite benannt ist. Nur so funktioniert das spätere Laden von Aufstellungen, TTR-Werten und Bilanzen von `click-TT`.

Es kann der externe Link zur `Vereinssuche` verwendet werden, um den genauen Namen des Vereins herauszufinden.

### Vereins-Nummer

Die Vereinsnummer kann auf der `click-TT` Info Seite des Vereins oder über die `Vereinssuche` herausgefunden werden.

Sie wird für das spätere Laden von Aufstellungen, TTR-Werten und Bilanzen von `click-TT` benötigt.
...
Zudem kann die Aufstellung aus der der Planung vorangegangen Halbserie von `click-TT` übernommen werden.

> **Achtung** Wurde bereits eine manuelle Planung begonnen, und nachträglich wird eine Aufstellung von `click-TT` geladen, so gehen die Spieler-Informationen der manuellen Planung verloren!
...
Es kann direkt die Aufstellung aus der der Planung vorangegangen Halbserie von `click-TT` übernommen werden.

Dazu klickt man in einer leeren Planung auf den entsprechenden Link oder verwendet den `CLICK-TT` Link oben rechts.

![Der Dialog zum Übernehmen einer Aufstellung von click-TT][planung-lade-aufstellung]

*Der Dialog zum Übernehmen einer Aufstellung von click-TT*

Es öffnet sich ein Dialog mit einem Webbrowser, welcher die Aufstellung der vorangeganen Halbserie der aktuellen Planung von `click-TT` anzeigt.
...
![Die von click-TT übernommene Aufstellung im Editor][planung-aufstellung-geladen]

*Die von click-TT übernommene Aufstellung im Editor*

### 4.2 Aktualisieren der TTR-Werte

Wurde eine Aufstellung von `click-TT` geladen, so können die TTR-Werte dir Spieler aktualisiert werden.

Das geht über den `TTR` Button oben rechts.

> **Hinweis** Zum Laden der TTR-Werte wird ein Account benötigt.
...
> **Hinweis** Zunächst wird dazu aufgefordert sich einzuloggen. Nachdem dies erfolgt ist, lädt die QTTR-Rangliste des Vereins.
...
Das funktioniert genau so wie das Einloggen in einem normalen Webbrowser.

> **Hinweis** Als Premium Mitglied, kann man über den Rangliste Filter die aktuellen TTR-Werte laden.
...
Diese werden mit der Aufstellung von `click-TT` importiert und können über den Spieler Dialog (klicke auf einen Spieler) gesetzt werden.
...
Als Hilfe für die Aufstellung können von `click-TT` auch die Bilanzen vergangener Halbserien übernommen werden.
